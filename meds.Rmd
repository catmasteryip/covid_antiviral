...
ip.molnupiravir = ip.molnupiravir %>%
  ...
  filter(daystillmolnupiravir > 0,
         as.Date(`Dispensing Date (yyyy-mm-dd)`) >= as.Date(`Admission Date (yyyy-mm-dd)`)) %>% 
  group_by(`Reference Key`) %>%
  summarise(daystillmolnupiravir = min(daystillmolnupiravir)) 
...
```{r}
ip.drug.all = bind_rows(ip.drug.imputed,
                        mutate(dexamethasone_shortlist, dexamethasone = 1),
                        mutate(baricitinib_shortlist, baricitinib = 1),
                        mutate(ip.paxlovid, paxlovid = 1),
                        mutate(ip.molnupiravir, molnupiravir = 1)) %>%
  select(-matches("^days"))
