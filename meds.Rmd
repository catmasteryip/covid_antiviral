...
ip.paxlovid = ip.paxlovid %>%
  ...
  # get earliest dispension of antiviral by reference key
  group_by(`Reference Key`) %>%
  slice(which.min(ymd(`Admission Date (yyyy-mm-dd)`))) %>%
  ungroup()
...
```{r}
ip.drug.all = ip.drug.imputed %>%
  inner_join(mutate(dexamethasone_shortlist, dexamethasone = 1)) %>%
  inner_join(mutate(baricitinib_shortlist, baricitinib = 1)) %>%
  inner_join(mutate(ip.paxlovid, paxlovid = 1)) %>%
  inner_join(mutate(ip.molnupiravir, molnupiravir = 1)) %>%
  select(-matches("^days"))
