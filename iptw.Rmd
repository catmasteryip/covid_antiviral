...
ip.iptw.p.organ_dys.orig = ip.iptw.p %>%
    left_join(organ_dys)
ip.iptw.p.organ_dys.orig = ip.iptw.p.organ_dys.orig %>%
...
for(daystillevent in c("Circulatory.shock","Respiratory.failure","Acute.kidney.injury","Coagulopathy","Acute.liver.impairment")){
  ...
  hr.death.small = iptw_hr_tidy(df = ip.iptw.p.organ_dys,
                          x_factors = x_factors_hr,
                          event = "event",
                          daystillevent = daystillevent,
                          treatment = "paxlovid") %>%
    mutate(event = daystillevent)
  hr.death.big = bind_rows(hr.death.big, hr.death.small)
  
}
writexl::write_xlsx(hr.death.big, path = "./results/hr/hr.organ_dys.agr_grp5.p.xlsx")
```
...
for(daystillevent in c("Circulatory.shock","Respiratory.failure","Acute.kidney.injury","Coagulopathy","Acute.liver.impairment")){
  ...
  hr.death.small = iptw_hr_tidy(df = ip.iptw.m.organ_dys,
                          x_factors = x_factors_hr,
                          event = "event",
                          daystillevent = daystillevent,
                          treatment = "molnupiravir") %>%
