...
ip_dexa = ip_dexa %>%
  ...
  mutate(dexamethasone = ifelse(!is.na(daystilldexa), 1, 0),
         daystilldexa = ifelse(is.na(daystilldexa), 28, daystilldexa),
...
ip_icu = ip_tab1_df  %>%
  ...
  select(-`HN Number`)
ip_icu = ip_icu %>%
  # replace daystillicu NA, impute icu, factorise
  mutate(icu = ifelse(is.na(icu), 0, icu),
         daystillicu = ifelse(is.na(daystillicu), 28, daystillicu),
         across(age_group:icu, factor),
         age_group = ifelse(age_group != "under65", "65plus", age_group))
...
x_factors = colnames(ip_icu)[!colnames(ip_icu) %in% c("daystillicu", "icu")]
# iterate over cols
for(x in x_factors){
  
  iformula <- as.formula(sprintf("Surv(as.numeric(daystillicu), as.numeric(icu)) ~ %s", x))
  coxfit_dexa <- coxph(formula = iformula, 
                data = ip_icu)
