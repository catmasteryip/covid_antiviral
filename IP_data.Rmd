...
for(name in colnames(ip_propensity)){
  
}
weight <- ipwpoint(exposure = paxlovid, family = "binomial", link = "logit",
  numerator =~ 1,
  denominator =~ diabetes ,
  trunc = 0.01, data = ip_propensity) 
...
tidy(res, exponentiate = T, conf.int = T, conf.level = .95)
```
