...
ip_drug_use = ip_drug_use %>%
  impute_meds() %>%
  left_join(pres_date) 

ip.paxlovid.daystillpaxlovid = ip_drug_use %>%
  filter(paxlovid == 1) %>%
  ...
  group_by(`Reference Key`) %>%
  summarise(daystillpaxlovid = min(daystillpaxlovid)) 

ip.paxlovid.daystillpaxlovid %>% count()

ggplot(ip.paxlovid.daystillpaxlovid, aes(x = daystillpaxlovid)) +
  geom_histogram()
```

# medication imputation cont'd
